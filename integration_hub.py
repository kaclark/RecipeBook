...
def scrape_int(task_obj):
    print(task_obj.content)
    intscrape(task_obj.description.split('(')[-1].split(')')[0])
...
def int_choice_boson(task_obj):
    ...
    if len(fibres) > 0:
        choices = ["choose_from_fibres", "Integrate", "View"] 
    else:
        choices = ["Integrate", "View"] 

    forward_parameters = {

            "choose_from_fibres": (browse_tasks, fibres), 
